...
export function RegisterPage({ onNavigateLogin, onSuccess }: RegisterPageProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      await register(email, name, password, languages)
      onSuccess()
    } catch (err: any) {
      ...
      <div className="w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            <label className="text-foreground text-sm font-medium">Idiomas que você fala</label>
            <p className="text-muted-foreground text-xs -mt-1">Selecione um ou mais idiomas</p>
        ...
        <p className="text-center text-muted-foreground text-sm mt-6">
          Já tem conta?{' '}
          <button onClick={onNavigateLogin} className="text-primary hover:underline">
            Faça login
          </button>
        </p>
      </div>
